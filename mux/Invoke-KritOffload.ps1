...
.PARAMETER Route
  'auto' (default) | 'lmstudio' | 'kritical-openrouter' | 'kritical-nvidia' | 'kritical-mistral'.
  'auto' tries LM Studio (Tier 1) first, then falls to 'kritical-openrouter' on the hosted
  container (the Tier-2 route proven reliable as of 2026-08-01 — see the HOWTO doc's per-route
  PASS/FAIL table before relying on kritical-mistral). 'kritical-coding', 'kritical-scx', and any
  'kritical-thegrid*' name are DELIBERATELY NOT in this set — see the thegrid-budget guard above.
...
.PARAMETER LmStudioBase
  Tier 1 endpoint. Default http://127.0.0.1:1234/v1 — the LOCAL address LM Studio's OpenAI-
  compatible server listens on. On the CURRENT (2026-08-01) architecture this is only reachable
  from the laptop once (a) the W365 box has LiteLLM running next to LM Studio, and (b) ONE port
  (LiteLLM's, not LM Studio's) is tunneled/forwarded to this address. Do NOT tunnel LM Studio's
  1234 directly to the laptop — that would push raw GPU inference traffic over the tunnel; tunnel
  LiteLLM instead and let it reach LM Studio over localhost on the W365 box itself.
...
param(
    ...
    [string]$System,
    [switch]$Sensitive,
    [ValidateSet('auto','lmstudio','kritical-openrouter','kritical-nvidia','kritical-mistral','nvidia-direct','openrouter-direct','mistral-direct')]
    [string]$Route = 'auto',
...
function Get-KritOffloadLmStudioStatus {
    <#
      ...
      .NOTES
        ...
        disk for it.
    #>
    param([string]$BaseUrl = 'http://127.0.0.1:1234/v1')
...
if ($StatusOnly) {
    ...
    $report = @(
        [pscustomobject]@{ Provider = 'lmstudio (local NPU or W365 tunnel)'; State = $lmStatus.State; Detail = "binary=$($lmStatus.BinaryPresent) models=$($lmStatus.ModelsOnDiskCount)($($lmStatus.ModelsOnDiskGB)GB) port1234=$($lmStatus.PortOpen)"; StartCommand = $(if ($lmStatus.State -eq 'INSTALLED-NOT-RUNNING') { $lmStatus.StartCommand } else { '' }) }
...
if ($Sensitive) {
    if ($lmReachable) {
        ...
        $refusalReason = "SENSITIVE payload and Tier 1 (LM Studio) is UNREACHABLE at $LmStudioBase (W365 tunnel not up / LM Studio server not started). " +
                          "Refusing to send — sensitive payloads are NEVER downgraded to a hosted provider."
    }
} elseif ($Route -ne 'auto') {
    if ($Route -eq 'lmstudio') {
        if ($lmReachable) { $resolvedRoute = 'lmstudio' }
        else { $refusalReason = "Explicit -Route lmstudio requested but Tier 1 is UNREACHABLE at $LmStudioBase." }
    ...
    elseif ($directStatus['mistral-direct'].Status -eq 'OK') { $resolvedRoute = 'mistral-direct' }
        ...
        $reasons = @(
            "lmstudio ($LmStudioBase): unreachable"
...
$isHostedRoute = ($resolvedRoute -and $resolvedRoute -ne 'lmstudio')
